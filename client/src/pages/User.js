  ...
  useEffect(() => {
    if (recipes.length > 0 && userData?.id) {
      const count = recipes.filter(
        (recipe) => recipe.author_id.toString() === userData.id.toString()
      ).length;
      setAuthoredRecipesCount(count);
      console.log(
        `Number of recipes authored by user ${userData.id}:`,
        authoredRecipesCount
      );
    }
  }, [recipes, userData.id]);

  const calculateAuthoredRecipes = () => {
    const authoredRecipesCount = recipes.filter(
      (recipes) => recipes.author_id === userId
    ).length;
